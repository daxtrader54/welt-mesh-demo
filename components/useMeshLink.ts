...
export type MeshLinkHandlers = {
  ...
  /** Fired once the token is in hand, before the overlay opens. */
    ...
    async (
      ...
      try {
        ...
        const link = createLink({
          ...
          onTransferFinished: payload => latest.current.onTransferFinished(payload),
          onExit: (error, summary) => latest.current.onExit(error, summary)
        })

        link.openLink(json.linkToken)
      } catch (err) {
